...
def introduce_program():
    ...
    image_name = input("Welcome to STEGosaurus! Enter the name of your image in the example folder to get started ("
                       "Don't include file extension): ")
    name_of_new_image = input("Enter the name of your new image, it will be saved in the example folder of this "
                              "project: ")
...
if __name__ == '__main__':
    input(
        "Hello, there! The original image is in the example folder and is named steg and the secret message is in "
        "sus_steg. Press any key to see what the message!")
    print(decode_image())
    # Unpack basic data
    name_of_image, new_image_name, message = introduce_program()
    path_to_image = f"example/{name_of_image}.png"  # Original image path
    ...
    new_yaml_location = f"example/{new_image_name}_secrets.yaml"
